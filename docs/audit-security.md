...
| S5/S11 (September 17) | Persist exact signed Solana bytes before send, retain attempts without TTL, reconcile history, park unresolved expiry, and delete definitive terminal evidence atomically with queue completion. Serialized input tracks its own hash. Automatic re-signing is disabled. | Redis/RPC fault tests and actual-validator crash/restart; [recovery contract](design/rpc-and-solana-recovery.md). |
| Solana signing (September 17) | Separate local Ed25519 key reference, operator token, SDK-compatible wire encoding, required-signature validation and secret-free queue identity. | Independent signature/wire tests and actual HTTP authorization/admission tests. |
| Configured RPC access (September 17) | Per-chain provider credentials stay in configuration; protected read/sign/write routes, pooled connections, redacted diagnostics and explicit Base preconfirmation capability. | Local HTTP provider tests and authenticated testnet reads; [verification](verification.md). |
| S6, partial | Empty/default-auto requests and unsupported account contract creation return validation errors before writes. Explicit fee parsing preserves legacy/dynamic/7702 fields and rejects conflicts. | API validation regressions; request/storage round trips and actual worker signing produce the expected legacy wire envelope and exact gas price. Solana malformed wire, payer mismatch, compute-budget override and unsupported automatic blockhash retry now reject before admission. |
...
| EOA fee limits (September 17) | Explicit caller fees remain caps during stalled recovery; unsupported dynamic-fee estimation cannot silently select uncapped legacy pricing. Failed recovery preserves intent instead of sending a fallback self-transfer or resetting nonces. | Four signed-wire and Redis/HTTP regressions; [fee contract](design/eoa-fee-recovery.md). Automatic estimates still need an operational spending policy. |
| S10, EOA execution status (September 17) | A mined revert becomes terminal `failed`, emits `confirm/failure` with `TRANSACTION_REVERTED`, retains its receipt and replay identity, and consumes its nonce without retry. | Real Redis lifecycle/webhook tests cover reverted and successful receipts; [execution-status contract](design/eoa-execution-status.md). Finality/reorg and other executor semantics remain separate work. |
...
The initial checks passed locally on 2026-09-15; dated rows describe later additions. S10 and the remaining portions of S6/S12/S13 still require work. Solana historical ambiguity is contained by pausing, rather than resolved automatically. Stable replay identity prevents fresh-nonce/UID duplication; it does not establish chain finality or recover every ambiguous provider outcome.
